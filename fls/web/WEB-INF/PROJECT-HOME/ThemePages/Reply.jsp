    ...
    <body onload="login_onload_yes('${request.yesmsg}');login_onload_no('${request.nomsg}');">
        ...
        <div id="header_wrap">
            ...
            <div id="header">
                ...
                <!-- begin  search -->
                <div id="search">
                    <s:form action="ThemeAction!queryThemeByKey" theme="xhtml" namespace="/theme" method="post" name="searchform" >          
        ...
        <div id="wrap">
            ...
            <div id="wrap_left">
                ...
                <div id="wrap_info">
                    <div id="pub_wrap" style="display: none;">
                        <s:form   theme="xhtml" action="ThemeAction!sendTheme" namespace="/theme" method="post" name="sendlsform" onsubmit="return check_sendls();">          
                ...
                <div class="infobox">
                    <div class="c">
                        <a href="<%=basePath%>theme/ReplyAction!queryReply?currentPage=1&theme_id=${theme.theme_id}">
                            <span><s:property escape="false" value="#request.theme.theme_content" /></span>
                        </a>	 
                    </div>
                    <div class="date">
                        <div id="dateleft">
                            <a href="<%=basePath%>theme/ReplyAction!queryReply?currentPage=1&theme_id=${theme.theme_id}">查看</a> 评论(<font color="red"><s:property value="#request.pageEntity.allRow"/></font>)
                        </div>
                        <div id="dateright">
                            ...
                            <br>
                            <font color="red"><s:date name="#request.theme.theme_updatetime" format="yyyy-MM-dd HH:ss:mm"/> </font>
                            &nbsp;

                            <a href="<%=basePath%>user/UserInfoAction!viewUserInfo?viewflag=index&viewuser_id=<s:property value="#request.theme.theme_owner.user_id"/>">
                                <font color="blue"><s:property value="#request.theme.theme_owner.user_name"/></font>
                            </a>
                            &nbsp;&nbsp;&nbsp;
                            <a href="<%=basePath%>theme/ThemeAction!queryThemeByKey?searchKey=<s:property value="#request.theme.theme_category"/>&currentPage=1&chgEncoding=YES"> 
                                <font color="green"><s:property value="#request.theme.theme_category"/> </font>
                            </a>
                            &nbsp;&nbsp;&nbsp;
                            <span id="fav_${request.theme.theme_id }">
                                <a onclick="addtofav('${request.theme.theme_id }')" style="cursor: pointer">+加入收藏</a>
                            </span>

                            <a title="分享到人人网"  href="">
                                <img align="absmiddle" src="Images/renren.jpg" width="16" height="16" border="0" alt="分享到人人网">
                            </a> 
                            <a title="分享到新浪微博"  href="">
                ...
                <div id="cbox2">
                    <div id="cbox2_l"></div>
                    <div id="cbox2_c"><B>评论</B></div>
                    <div id="cbox2_r"></div>

                </div>
                <div id="wrap_comment">	
                    <s:iterator value="pageEntity.list" var="reply" >
                        <div class="comment_box">
                            <div class="comment_body"><s:property value="#reply.reply_content"/></div>
                            <div class="comment_options">
                                <s:if test="#reply.reply_owner.user_imageurl!=null">
                                    
                                    <s:if test="#reply.reply_owner.imageFlag==false">
                                        <img src="<%=basePath%>user/PhotoAction?viewuser_id=${reply.reply_owner.user_id}" width="24" height="24">
                                     </s:if>
                                     <s:else>
                                         <img src="<%=basePath%>Images/faces/${reply.reply_owner.user_imageurl}" width="24" height="24">
                                    </s:else>
                                   
                                </s:if>
                                <s:else>
                                    <img src="Images/list_m.gif" width="24"> 
                                </s:else>
                                <span><s:property value="#reply.reply_index"/>#</span>
                                <font color="red"> <s:date name="#reply.reply_newtime" format="yyyy-MM-dd HH:ss:mm"/> </font> by  

                                <a href="<%=basePath%>user/UserInfoAction!viewUserInfo?viewflag=index&viewuser_id=<s:property value="#reply.reply_owner.user_id"/>">
                                    <font color="blue"><s:property value="#reply.reply_owner.user_name"/></font>
                                </a>
                            </div>
                        </div>
                    </s:iterator>
                        ...
                        <s:if test="%{pageEntity.list.size>0}">
                            <div id="paging" style="width:600px;">
                                <span> 
                                    ...
                                    <a> 当前第<font color="red"><s:property value="pageEntity.currentPage"/></font>页</a> 
                                </span>
                                <span>
                                    ...
                                    <s:else>
                                        <a href="<%=basePath%>theme/ReplyAction!queryReply?currentPage=1">第一页</a>
                                        <a href="<%=basePath%>theme/ReplyAction!queryReply?currentPage=<s:property value="%{pageEntity.currentPage-1}"/>">上一页</a> 
                                    </s:else>
                                    <s:if test="%{pageEntity.currentPage != pageEntity.totalPage}">
                                        <a href="<%=basePath%>theme/ReplyAction!queryReply?currentPage=<s:property value="%{pageEntity.currentPage+1}"/>">下一页</a>
                                        <a href="<%=basePath%>theme/ReplyAction!queryReply?currentPage=<s:property value="pageEntity.totalPage"/>">最后一页</a>
                                    </s:if>
                                    <s:else>
                                        <a>下一页</a>  
                                        <a>最后一页</a>
                ...
                <s:if test="#session.LOGIN_USER.user_id!=null">
                    <div id="post_comment">
                        ...
                        <div id="post_comment_box">
                            <s:form  theme="xhtml"action="ReplyAction!sendReply" namespace="/theme" method="post" name="replyform" onSubmit="return check_reply_Submit();"> 
                                <input name="uid" type="hidden">            
                                <input name="info_id" id="info_id" type="hidden" value="${theme.theme_id}">               
                                <input name="comment_num" id="comment_num" type="hidden" value="<s:property value="#request.theme.theme_replys.size+1"/>"/>            
                                <textarea name="post_comment_text" id="post_comment_text" ></textarea>          
                                <label>昵称 <input name="post_comment_name" id="post_comment_name" type="text" value="${session.LOGIN_USER.user_name }" readonly="readonly"></label>
                                <input id="post_comment_btn" accesskey="s" type="submit" value="发布评论(Alt+S)">
                            </s:form>
                        </div>
                    </div>
                </s:if>
                <s:else>
                    <div id="post_comment">
                        <H3>登录后才可以发布评论 
                            <a href="<%=basePath%>NavigateAction?navFlag=login"><b>登录</b></a>
                        </H3>
                    </div>
                </s:else>
            ...
            <div id="wrap_right">
                ...
                <script type="text/javascript" id="bdshare_js" data="type=tools&amp;uid=3495715" ></script>
